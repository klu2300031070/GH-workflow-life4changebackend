...
public class BloodBankServiceimpl implements BloodBankService {
	...
	public String validateAndSave(BloodDonor donor) {
	    ...
	    Optional<BloodDonor> lastRecordOpt =
	        br.findTopByAddharOrderByCurrentDonationDateDesc(donor.getAddhar());

	    if (lastRecordOpt.isPresent()) {

	        BloodDonor last = lastRecordOpt.get();

	        // 🔥 FIX: convert String → LocalDate
	        LocalDate lastDate = LocalDate.parse(last.getCurrentDonationDate(), formatter);
	        ...
	        if ("Female".equalsIgnoreCase(last.getGender()) && days < 120) {
	            return "Female donor must wait 120 days (Last: " + days + " days ago)";
	        }

	        if ("Male".equalsIgnoreCase(last.getGender()) && days < 90) {
	            return "Donor must wait 90 days (Last: " + days + " days ago)";
	        }

	        donor.setDaysSinceLastDonation(days);
	    }

	    // 🔥 store today's date as string
	    donor.setCurrentDonationDate(
	        LocalDate.now().format(formatter)
	    );

	    br.save(donor);
	    BloodData bd=bdr.findByOrgAndType(donor.getOrg(),donor.getBloodType());
	    bd.setDonatedunits(bd.getDonatedunits()+1);
	    bd.setAunits(bd.getAunits()+1);
	    bdr.save(bd);
